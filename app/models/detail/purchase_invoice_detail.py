...
class PurchaseInvoiceDetail(BaseModel, BaseMixin):
    ...
    # === NEW Phase D — Catatan Purchase Invoice §3: UOM on detail ===
    satuan_id = Column(UUID(as_uuid=True), ForeignKey("satuan.id", name="fk_pinvoice_detail_satuan"), nullable=True, index=True)

    # Relationships
    purchase_invoice = relationship("PurchaseInvoice", back_populates="details")
    barang = relationship("Barang")
    satuan = relationship("Satuan", foreign_keys=[satuan_id])  # Phase D
    # Source-line trace
