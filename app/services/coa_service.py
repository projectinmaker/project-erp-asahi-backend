...
def get_coa_list(
    ...
    """Mengambil daftar COA dengan filter opsional"""
    ...
    if search:
        ...
        query = query.filter(
            or_(AkunPerkiraan.kode.ilike(search_pattern), AkunPerkiraan.nama.ilike(search_pattern))
        )

    return query.order_by(AkunPerkiraan.kode).offset(skip).limit(limit).all()
