...
def create_penyesuaian(
    ...
    try:
        ...
        db.commit()
        db.refresh(adj)
        logger.info(f"PenyesuaianStok created: {no_adj} | tipe={tipe} | total={total} | expiry={tanggal_kedaluwarsa}")
        return adj
...
def update_penyesuaian(
    ...
    **kwargs,
    ...
    Phase A fix: tanggal_kedaluwarsa dikelola via **kwargs.
    - Kalau field dikirim (baik date atau null) → set di obj (null = clear expiry)
    - Kalau field tidak dikirim → skip, pertahankan existing
    """
    ...
    # Phase A: tanggal_kedaluwarsa via kwargs
    # Hanya set kalau explicitly dikirim (baik date atau None untuk clear)
    if 'tanggal_kedaluwarsa' in kwargs:
        db_obj.tanggal_kedaluwarsa = kwargs['tanggal_kedaluwarsa']
...
def approve_penyesuaian(db: Session, db_obj: PenyesuaianStok) -> PenyesuaianStok:
    ...
    Phase A — Catatan Audit Inventory:
    - Enforce gudang_id required (item 7: "gudang_id nullable pada physical stock transactions")
    - Pass tanggal_kedaluwarsa for FEFO (item 5: "FEFO incoming requires expiry")
    - FEFO + TAMBAH requires tanggal_kedaluwarsa (item 5)
    """
    ...
    # === Phase A: Enforce gudang_id required ===
    ...
    if barang:
        ...
        if metode == 'FEFO' and db_obj.tipe.value == 'TAMBAH' and not db_obj.tanggal_kedaluwarsa:
            raise ValueError(
                ...
                f"Penyesuaian TAMBAH untuk barang FEFO wajib mengisi tanggal_kedaluwarsa "
                f"(Catatan Audit Inventory §3 item 5)."
            )
    ...
    movement = update_stok_barang(
        ...
        ref_id=db_obj.id,
        # Phase A: pass expiry for FEFO
        tanggal_kedaluwarsa=db_obj.tanggal_kedaluwarsa if mode == "TAMBAH" else None,
    )
